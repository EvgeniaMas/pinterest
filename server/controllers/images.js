...
exports.show = function (req, res) {
    ...
    Images.find().sort('-created').populate('user', 'local.name').exec(function(error, images) {
        if (error) {
        ...
        res.render('images-gallery', {
            ...
            images: images,
            gravatar: gravatar.url(images.email ,  {s: '80', r: 'x', d: 'wavatar'}, true)
        });
...
exports.update = function(req, res) {   
   Images.findByIdAndUpdate(req.params.id, {$inc: { likes: 1}} , function(err){
        if (err) throw err;
        res.redirect('/images-gallery');
    });
};
